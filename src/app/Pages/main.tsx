...
// Define the structure for the windows state
...
interface AppData {
  ...
  icon: string;
  action: string;
  subAction?: string; // Optional field
  size: string; // Class name for styling
}
    ...
    <>
      ...
      <div className="relative h-screen" ref={constraintsRef}>
        <div className="relative h-full w-full top-0 left-0 z-10 text-white">
          <RightClick option={true} />
          <div className="grid h-[80vh] grid-rows-8 gap-2 absolute top-2 left-2">
            {appsData.map((app: AppData, index: number) => (
              <motion.div
                key={app.id}
                drag
                dragConstraints={constraintsRef}
        ...
        >
          ...
          <Explorer
            isExplorerOpen={windows.explorer}
            toggleExplorer={(input: string | null | undefined) =>
              toggleWindow("explorer", input)
            }
            aboutMe={aboutMe}
            bounds={bounds}
          />
